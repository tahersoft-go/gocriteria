...
func TestValidateBody(t *testing.T) {
	...
	t.Run("Test Alpha", func(t *testing.T) {
		...
		schema := Schema{
			"name": New("name").Alpha(locales.EnUS).Required(),
		}
		type Payload struct {
...
func TestSetDefaultErrorMessages(t *testing.T) {
	SetDefaultErrorMessages(&message.Validations{
		IsEmail: "test",
	})
	if message.Default.IsEmail != "test" {
		t.Error("custom error message is not set properly")
	}
}

func TestSetFieldLabels(t *testing.T) {
	SetFieldLabels(&message.Labels{
		"name": "test",
	})
	name, ok := (*message.FieldLabels)["name"]
	if !ok || name != "test" {
