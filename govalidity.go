...
import (
	...
	"github.com/tahersoft-go/gocriteria/body"
...
func isSliceOfString(slice []interface{}) bool {
	...
	for _, sliceItem := range slice {
		_, err := convertor.ToNumber(sliceItem)
		if err != nil {
			isString = true
		}
...
func convertToSliceOfNumber(slice []interface{}) ([]float64, error) {
	result := []float64{}
	for _, sliceItem := range slice {
		num, err := convertor.ToNumber(sliceItem)
		if err != nil || num == nil {
...
func SetDefaultErrorMessages(v *message.Validations) {
	message.SetMessages(v)
}

func SetFieldLabels(l *message.Labels) {
	message.SetFieldLables(l)
}

func (v *Validator) run() []error {
	errs := []error{}
	for _, validation := range v.Validations {
		number, errConv := convertor.ToNumber(v.Value)
		str := ""
